...
static void s907x_mbedtls_free(void * ptr)
{
	wl_free(ptr);
}

static void* s907x_mbedtls_calloc(size_t nmemb, size_t size)
{
	void *mem;
...
static void wlan_task(void *context)
{
    ...
    if( err )
    {
        USER_DBG("kv init failed, err:%d",err);
    } 
	...
	mbedtls_platform_set_calloc_free(s907x_mbedtls_calloc, s907x_mbedtls_free);
	USER_DBG("wlan start..."); 
	printf("heap size %d\n", xPortGetFreeHeapSize());
    ...
    if(get_s907_run_mode() == s907x_mode_normal){
        //user app
#if ZG_BUILD
        wl_create_thread("zg main thread", MAIN_TASK_STACK_SZ, MAIN_TASK_PRIO, (thread_func_t)user_task, NULL);
#else
        wl_create_thread("main thread", MAIN_TASK_STACK_SZ, MAIN_TASK_PRIO, (thread_func_t)main_task, NULL);
#endif
...
const board_pin_t board_pin[]=
{
    ...
    {PIN6,              PIN_GPIO,      GPIO7,          GPIO7_AFP_GPIO,             GPIO_MODE_OUTPUT,       GPIO_NOPULL,        GPIO_NOPULL},
